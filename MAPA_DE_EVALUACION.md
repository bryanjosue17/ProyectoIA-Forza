...
| **Autenticación Keycloak** | `k8s/keycloak.yaml`<br>`k8s/keycloak-realm-configmap.yaml` | SSO con Authorization Code + PKCE. Configuración pre-cargada con roles (`employee`, `hr`, `admin`, etc.). |
...
| **Kubernetes (Manifiestos)** | `k8s/` (Raíz)<br>`[Submódulos]/k8s/` | Manifiestos completos (Deployments, Services, ConfigMaps, Secrets, Probes) listos para Docker Desktop. |
